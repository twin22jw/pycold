...
int lasso_blist_planet[NUM_LASSO_BANDS_PLANET] = {0, 1, 2, 3};
...
)
{
    ...
    char FUNC_NAME[] = "stand_procedure_planet";
...
//    }
    ...
    v_start = (float *)calloc(NUM_LASSO_BANDS_PLANET, sizeof(float));
    if (v_start == NULL)
    {
        RETURN_ERROR("ERROR allocating v_start memory", FUNC_NAME, FAILURE);
    }

    v_end = (float *)calloc(NUM_LASSO_BANDS_PLANET, sizeof(float));
    if (v_end == NULL)
    {
        RETURN_ERROR("ERROR allocating v_end memory", FUNC_NAME, FAILURE);
    }

    v_slope = (float *)calloc(NUM_LASSO_BANDS_PLANET, sizeof(float));
    if (v_slope == NULL)
    {
        RETURN_ERROR("ERROR allocating v_slope memory", FUNC_NAME, FAILURE);
    }

    v_dif = (float *)calloc(NUM_LASSO_BANDS_PLANET, sizeof(float));
    if (v_dif == NULL)
    {
        RETURN_ERROR("ERROR allocating v_dif memory", FUNC_NAME, FAILURE);
    }

    tmpcg_rmse = (float *)calloc(NUM_LASSO_BANDS_PLANET, sizeof(float));
    if (tmpcg_rmse == NULL)
    {
...
//    }
    ...
    v_diff = (float **) allocate_2d_array(NUM_LASSO_BANDS_PLANET,
...
//                    printf("rmse for band %d is %f\n", i_b, rmse[i_b]);
                ...
                for (i_b = 0; i_b < NUM_LASSO_BANDS_PLANET; i_b++)
                {
                    ...
                    mini_rmse = max((float)min_rmse[lasso_blist_planet[i_b]], rmse[lasso_blist_planet[i_b]]);
                    ...
                    v_start[i_b] = rec_v_dif[lasso_blist_planet[i_b]][0]
                            / mini_rmse;
                    ...
                    v_end[i_b] = rec_v_dif[lasso_blist_planet[i_b]][i-i_start]
                                            / mini_rmse;
                    ...
                    /*                                            */
                    /**********************************************/
                    v_slope[i_b] = fit_cft[lasso_blist_planet[i_b]][1] *
...
//                            fit_cft[b][1], fit_cft[b][2], fit_cft[b][3], fit_cft[b][4], fit_cft[b][5], fit_cft[b][6], fit_cft[b][7]);
                ...
                if (i_start > i_break)
                {
                    ...
                    for(i_ini = i_start - 2; i_ini >= i_break - 1; i_ini--) // SY 09192018
                    {
                        ...
                        v_diff_tmp = (float **) allocate_2d_array(NUM_LASSO_BANDS_PLANET,
                        ...
                        for (i_conse = 1; i_conse < ini_conse + 1; i_conse++) // SY 09192018
                        {
                            ...
                            for (i_b = 0; i_b < TOTAL_IMAGE_BANDS; i_b++)
                            {
                                ...
                                for (b = 0; b < NUM_LASSO_BANDS_PLANET; b++)
                                {
                                   if (i_b == lasso_blist_planet[b])
                                   {
                                        /**************************/
                                        /*                        */
                                        /* Minimum rmse.          */
...
//                                        printf("v_dif_mag: %f\n",v_dif_mag[i_b][i_conse]);
                        ...
                        mean_angle = MeanAngl_float(v_diff_tmp, NUM_LASSO_BANDS_PLANET, ini_conse);
                        /******************************************/
                        /*                                        */
                        /* Change detection.                      */
                        /*                                        */
...
//                                    CMdirection_outputs[current_CM_n] = tmp_direction;
                        ...
                        else if (vec_magg[0] > T_MAX_CG_PLANET) /* false change */
                        {
...
//            }
            ...
            if (bl_train == 1)
            {
                ...
                else // update frequency
                {
                    ...
                    for (b = 0; b < NUM_LASSO_BANDS_PLANET; b++)
                    {
                        matlab_2d_array_norm_float(rec_v_dif_copy, lasso_blist_planet[b], n_rmse,
                                         &tmpcg_rmse[b]);
    ...
    //                       }
                ...
                mean_angle = MeanAngl_float(v_diff, NUM_LASSO_BANDS_PLANET, adj_conse);
...
//                        rec_cg[*num_fc].t_confirmed = clrx[end - 1];
                ...
                else if (vec_mag[0] > T_MAX_CG_PLANET)  /*false change*/
                {
                    /**********************************************/
                    /*                                            */
                    /* Remove noise.                              */
    ...
    if (bl_train == 1)
    {
        ...
        {
            v_diff_tmp =(float **) allocate_2d_array(NUM_LASSO_BANDS_PLANET, adj_conse - i_conse, sizeof (float));
            for (k = 0; k < NUM_LASSO_BANDS_PLANET; k++)
                for(j = 0; j < adj_conse - i_conse; j++)
                  v_diff_tmp[k][j] = v_diff[k][i_conse+j];

            mean_angle = MeanAngl_float(v_diff_tmp, NUM_LASSO_BANDS_PLANET, adj_conse - i_conse);
...
******************************************************************************/
int cold_planet // DO I need to change this to cold_planet?
(
    long *buf_b,            /* I:  Landsat blue spectral time series.The dimension is (n_obs, 7). Invalid (qa is filled value (255)) must be removed */
    long *buf_g,            /* I:  Landsat green spectral time series.The dimension is (n_obs, 7). Invalid (qa is filled value (255)) must be removed */
    long *buf_r,            /* I:  Landsat red spectral time series.The dimension is (n_obs, 7). Invalid (qa is filled value (255)) must be removed */
...
)
{
    ...
    int *id_range;
    int i;
    char FUNC_NAME[] = "cold_planet"; // Do I need to change it to cold_planet?
    int result;
...
//    }
    ...
    if (clear_sum < N_TIMES * MAX_NUM_C){
        ...
        /*                                                            */
        /**************************************************************/
         result = stand_procedure_planet(valid_num_scenes, valid_date_array, buf_b, buf_g, buf_r, buf_n, buf_s1, buf_s2, buf_t, fmask_buf, id_range,
...
//         {
//             v_diff_tmp =(float **) allocate_2d_array(NUM_LASSO_BANDS_PLANET, adj_conse_end - i_conse, sizeof (float));
//             for (k = 0; k < NUM_LASSO_BANDS_PLANET; k++)
//                 for(j = 0; j < adj_conse - i_conse; j++)
//                   v_diff_tmp[k][j] = v_diff[k][i_conse+j];

//             mean_angle = MeanAngl_float(v_diff_tmp, NUM_LASSO_BANDS_PLANET, adj_conse_end - i_conse);
//             //float tt = vec_mag[i_conse];
//             if ((vec_mag[i_conse] <= adj_TCG)||(mean_angle >= NSIGN))
//             {
//                 /**************************************************/
...
//                         if (i_b == lasso_blist_planet[b])
//                         {
//                             status = auto_ts_fit_float(clrx_tmp, clry_tmp,  lasso_blist_planet[b], lasso_blist_planet[b], i_last_break,
//                                                        i_break_tmp - 1,
